...
urlpatterns = patterns('',
    ...
    url(r'^recommend/before/(?P<before>[0-9a-z]{,13})(?P<json>\.json)?$', 'archfinch.main.views.recommend', name='recommend-paged'),
    url(r'^recommend/(?P<category_slug>[a-z-]+)/(?P<before>[0-9a-z]{,13})(?P<json>\.json)?$', 'archfinch.main.views.recommend', name='recommend-slugged-paged'),
    url(r'^recommend(?:/(?P<category_slug>[a-z-]*))?/for/(?P<usernames>[,\w@\+\.-]+)(?:/(?P<before>[0-9a-z]{,13}))?(?P<json>\.json)?$', 'archfinch.main.views.recommend', name='recommend-for'),
    ...
    url(r'^item/(?P<item_id>[0-9a-z]{,13})(?:/(.*))?$', 'archfinch.main.views.item', name='item'),
    (r'^also_liked/(?P<item_id>[0-9a-z]{,13})/(?P<like>true|false)/(?P<also_like>true|false)$', 'archfinch.main.views.item_also_liked'),
    (r'^opinion/set/(?P<item_id>[0-9a-z]{,13})/(?P<rating>[1-5])$', 'archfinch.main.views.opinion_set'),
    (r'^opinion/remove/(?P<item_id>[0-9a-z]{,13})$', 'archfinch.main.views.opinion_remove'),

    url(r'^rss/followed/(?P<feed_username>[\w@\+\.-]+)$', LinkFeed(), {'followed': True}),
    url(r'^followed(?:/(?P<before>[0-9a-z]{,13}))?(?P<json>\.json)?$', 'archfinch.main.views.recommend', {'followed': True}, name='followed'),
    url(r'^tags/(?P<tag_names>.+)/before/(?P<before>[0-9a-z]{,13})(?P<json>\.json)?$', 'archfinch.main.views.recommend', name='fresh-tags-paged'),
    ...
    (r'^addtag/(?P<item_id>[0-9a-z]{,13})$', 'archfinch.main.views.add_tag'),
